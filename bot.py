...
# ───────── Flask 心跳伺服器 (Render / Railway 埠號綁定) ─────────
app = Flask(__name__)
    ...
    if os.path.exists(CONFIG_FILE):
        try:
            with open(CONFIG_FILE, "r", encoding="utf-8") as f:
                ...
                data.setdefault("github_storage", {
                    "enabled": False,
                    "token": "",
                    "repo": "",
                    "branch": "main",
...
# ───────── GitHub 儲存功能 ─────────
def get_github_storage_config():
    storage_cfg = config.get("github_storage", {}) or {}
    return {
        "enabled": bool(storage_cfg.get("enabled", False)),
        "token": str(storage_cfg.get("token", "") or os.environ.get("GITHUB_STORAGE_TOKEN", "")).strip(),
        "repo": str(storage_cfg.get("repo", "") or os.environ.get("GITHUB_STORAGE_REPO", "")).strip(),
        "branch": str(storage_cfg.get("branch", "main") or "main").strip(),
        "auto_sync": bool(storage_cfg.get("auto_sync", True)),
        "sync_interval": int(storage_cfg.get("sync_interval", 300) or 300),
    }

def load_data_sync_state():
    if os.path.exists(DATA_SYNC_FILE):
        try:
            with open(DATA_SYNC_FILE, "r", encoding="utf-8") as f:
                return json.load(f)
        except:
            pass
    return {"last_sync": "", "initialized": False}

def save_data_sync_state(state):
    try:
        with open(DATA_SYNC_FILE, "w", encoding="utf-8") as f:
            json.dump(state, f, indent=2, ensure_ascii=False)
    except:
        pass

async def github_api_request(method: str, endpoint: str, token: str, data: dict = None, timeout: int = 30):
...
async def github_create_repo(token: str, repo_name: str, private: bool = True):
    data = {
        "name": repo_name,
        "private": private,
        "auto_init": True,
        "description": "Discord AI Lover Bot Data Storage"
    }
    result = await github_api_request("POST", "/user/repos", token, data)
    if result and "error" not in result:
        return True, result.get("full_name", "")
    return False, result.get("error", "未知錯誤") if result else "連線失敗"

async def github_file_exists(token: str, repo: str, path: str, branch: str = "main"):
    endpoint = f"/repos/{repo}/contents/{path}"
    result = await github_api_request("GET", endpoint, token)
    if result and "error" not in result and isinstance(result, dict):
        return True, result.get("sha", "")
    return False, None

async def github_upload_file(token: str, repo: str, path: str, content: str, branch: str = "main", sha: str = None):
    endpoint = f"/repos/{repo}/contents/{path}"
    data = {
    ...
    if sha:
        data["sha"] = sha
    result = await github_api_request("PUT", endpoint, token, data)
    if result and "error" not in result:
        return True, None
    return False, result.get("error", "未知錯誤") if result else "連線失敗"

async def github_download_file(token: str, repo: str, path: str, branch: str = "main"):
    endpoint = f"/repos/{repo}/contents/{path}"
    result = await github_api_request("GET", f"{endpoint}?ref={branch}", token)
    if result and "error" not in result and isinstance(result, dict):
...
async def sync_to_github():
    storage_cfg = get_github_storage_config()
    if not storage_cfg["enabled"] or not storage_cfg["token"] or not storage_cfg["repo"]:
        return False, "GitHub 儲存未設定"
    
    token = storage_cfg["token"]
    repo = storage_cfg["repo"]
    branch = storage_cfg["branch"]
    ...
    for local_path, remote_path in files_to_sync:
        ...
        try:
            with open(local_path, "r", encoding="utf-8") as f:
                content = f.read()
            exists, sha = await github_file_exists(token, repo, remote_path, branch)
            ok, err = await github_upload_file(token, repo, remote_path, content, branch, sha if exists else None)
            if ok:
                success_count += 1
            else:
                errors.append(f"{remote_path}: {err}")
        except Exception as e:
            errors.append(f"{local_path}: {str(e)}")
    
    if success_count > 0:
        sync_state = load_data_sync_state()
        sync_state["last_sync"] = datetime.now(ZoneInfo("UTC")).isoformat()
        save_data_sync_state(sync_state)
        return True, f"已同步 {success_count} 個檔案"
    return False, "、".join(errors) if errors else "無檔案可同步"
...
async def sync_from_github():
    ...
    files_to_sync = [
        ...
        ("voice_profiles.json", VOICE_CONFIG_FILE),
    ]
    
    success_count = 0
    
    for remote_path, local_path in files_to_sync:
        ok, content = await github_download_file(token, repo, remote_path, branch)
        if ok and content:
            try:
    ...
    return success_count > 0, f"已載入 {success_count} 個檔案"

async def github_sync_worker():
    await client.wait_until_ready()
    while not client.is_closed():
        storage_cfg = get_github_storage_config()
        if not storage_cfg["enabled"] or not storage_cfg["auto_sync"]:
            await asyncio.sleep(60)
            continue
        
        interval = max(60, storage_cfg["sync_interval"])
        await asyncio.sleep(interval)
        
        try:
            ok, msg = await sync_to_github()
            if ok:
                print(f"[github_sync] {msg}")
        except Exception as e:
            print(f"[github_sync] 錯誤: {e}")

async def _initial_github_sync():
    await client.wait_until_ready()
    await asyncio.sleep(5)
    storage_cfg = get_github_storage_config()
    if storage_cfg["enabled"] and storage_cfg["token"] and storage_cfg["repo"]:
        print("[github_sync] 啟動時從 GitHub 同步資料...")
        ok, msg = await sync_from_github()
        if ok:
            print(f"[github_sync] {msg}")
            global config, voice_profiles
            config = load_config()
            voice_profiles = load_voice_profiles()
        else:
            print(f"[github_sync] 同步失敗: {msg}")
...
def get_voice_listen_config():
    ...
    return {
        ...
        "stay_connected": bool(cfg.get("stay_connected", False)),
        "stt_required": bool(cfg.get("stt_required", True)),
    }
...
async def _connect_voice_only(voice_channel_id: int):
    global voice_listen_task
    if not voice_channel_id:
        return False, "未提供語音頻道 ID"
    ...
    try:
        ...
        return True, f"已連接語音頻道：{channel.name}"
    except Exception as e:
        return False, f"連接失敗：{type(e).__name__}: {e}"
...
async def _ensure_voice_listen_on_ready():
    ...
    stt_cfg = get_stt_config()
    has_stt = bool(stt_cfg.get("url"))
    stt_required = cfg.get("stt_required", True)
    stay_connected = cfg.get("stay_connected", False)
    
    if not has_stt and stt_required and not stay_connected:
        print("[voice] STT 未設定且 stt_required=True，跳過語音連接")
        return
    
    if not has_stt or stay_connected:
        ok, msg = await _connect_voice_only(voice_id)
        if ok:
            print(f"[voice] {msg}（僅連接模式，無 STT）")
        else:
            print(f"[voice] 連接失敗：{msg}")
        return
    
    if not reply_id:
        print("[voice] reply_channel_id 未設定，使用僅連接模式")
        ok, msg = await _connect_voice_only(voice_id)
        if ok:
            print(f"[voice] {msg}")
        return
    ...
    try:
        ...
        print("[voice] 已啟動語音偵測模式")
    except Exception as e:
        print(f"[voice] auto-start failed: {type(e).__name__}: {e}")


async def _voice_keepalive_worker():
    global voice_manual_disconnect
    await client.wait_until_ready()
    ...
    while not client.is_closed():
        ...
        if not is_connected:
            ...
            if reconnect_attempts > max_reconnect_attempts:
                print(f"[voice_keepalive] 已達最大重連次數 ({max_reconnect_attempts})，停止重連")
                continue
            
            print(f"[voice_keepalive] 偵測到斷線，嘗試重連... (第 {reconnect_attempts} 次)")
            ...
            if not has_stt or stay_connected or not reply_id:
                ok, msg = await _connect_voice_only(voice_id)
                if ok:
                    print(f"[voice_keepalive] 重連成功：{msg}")
                    reconnect_attempts = 0
                    voice_manual_disconnect = False
                else:
                    print(f"[voice_keepalive] 重連失敗：{msg}")
            else:
                try:
                    await _start_voice_listen(voice_id, reply_id)
                    print("[voice_keepalive] 重連成功（語音偵測模式）")
                    reconnect_attempts = 0
                    voice_manual_disconnect = False
                except Exception as e:
                    print(f"[voice_keepalive] 重連失敗：{type(e).__name__}: {e}")
        else:
            reconnect_attempts = 0
...
def _detect_api_type(api_url: str) -> str:
    url = str(api_url or "").lower().strip()
...
def _normalize_api_base(api_url: str) -> str:
    ...
    api_type = _detect_api_type(base)
    if api_type == "google":
        return base
...
async def _fetch_google_models():
    ...
    try:
        ...
        pass
    return ["gemini-1.5-flash", "gemini-1.5-pro", "gemini-pro", "gemini-2.0-flash"]
...
class MyClient(discord.Bot):
    ...
    async def setup_hook(self):
        ...
        # 語音連接保持器（自動重連）
        self.loop.create_task(_voice_keepalive_worker())
        # GitHub 儲存同步任務
        self.loop.create_task(github_sync_worker())
        # 啟動時從 GitHub 同步資料
        self.loop.create_task(_initial_github_sync())
...
async def call_api(channel_id, user_text=None, special_instruction=None, author=None):
    ...
    api_type = _detect_api_type(config.get("api_url", ""))
    
    if api_type == "google":
        return await _call_google_api(channel_id, user_text, special_instruction, author)
    else:
        return await _call_openai_api(channel_id, user_text, special_instruction, author)

async def _call_openai_api(channel_id, user_text=None, special_instruction=None, author=None):
    endpoint = f"{config['api_url'].rstrip('/')}/chat/completions"
    headers = {
        "Authorization": f"Bearer {config['api_key']}",
        "Content-Type": "application/json"
...
async def _call_google_api(channel_id, user_text=None, special_instruction=None, author=None):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(endpoint, json=body, timeout=45) as resp:
                if resp.status != 200:
                    res_text = await resp.text()
                    return f"❌ API 錯誤 ({resp.status}): {res_text[:100]}"
                data = await resp.json()
                ...
                cleaned = _cleanup_response_text(content)
                if user_text and _is_song_request(user_text):
                    song_candidates = _extract_song_candidates(cleaned)
                    if song_candidates:
                        for item in song_candidates:
                            if item and item not in song_history:
                                song_history.append(item)
                        if len(song_history) > 20:
                            del song_history[:-20]
...
@discord.option("url", description="API URL", required=False)
@discord.option("key", description="API Key", required=False)
@discord.option("model", description="選擇模型", autocomplete=model_autocomplete, required=False)
async def set_api(
    interaction: discord.Interaction,
    url: str = None,
    key: str = None,
    ...
    if url or key:
        ...
        if models:
            model_list = "\\n".join(models[:20])
            total = len(models)
            extra = f"（還有 {total - 20} 個模型）" if total > 20 else ""
            await interaction.followup.send(
                "\\n".join([
                    "✅ API 設定已更新：" + "、".join(updated),
                    "",
                    f"📋 已自動取得 {total} 個可用模型：",
                    f"```",
                    model_list,
            ...
            await interaction.followup.send(
                "\\n".join([
                    "✅ API 設定已更新：" + "、".join(updated),
                    "",
                    "⚠️ 無法自動取得模型清單，請確認 API URL/Key 是否正確。",
                    "您可以直接使用 `/set_api model:模型名稱` 手動設定模型。"
                ]),
                ephemeral=True
            )
    else:
...
async def set_voice_listen(
    ...
    stay_connected: Optional[bool] = None,
    stt_required: Optional[bool] = None,
):
    global voice_manual_disconnect
    if not is_owner(interaction):
        return
    ...
    if status:
        ...
        await interaction.followup.send(
            "\\n".join([
                ...
                f"🔗 保持連接：{cfg.get('stay_connected', False)}",
                f"📋 STT 必要：{cfg.get('stt_required', True)}",
                f"🛰️ STT 服務：{stt_cfg.get('url') or '未設定'}",
                "",
                "ℹ️ 若 STT 未設定且 stt_required=False，機器人會僅連接語音頻道而不進行語音辨識。"
            ]),
            ephemeral=True
        )
        return

    if stay_connected is not None or stt_required is not None:
        current_cfg = config.get("voice_listen", {}) or {}
        if stay_connected is not None:
            current_cfg["stay_connected"] = stay_connected
        if stt_required is not None:
            current_cfg["stt_required"] = stt_required
        config["voice_listen"] = current_cfg
        save_config(config)
        await interaction.followup.send(
            "\\n".join([
                "✅ 已更新語音連接設定。",
                f"🔗 保持連接：{current_cfg.get('stay_connected', False)}",
                f"📋 STT 必要：{current_cfg.get('stt_required', True)}",
            ]),
            ephemeral=True
        )
        return
...
@client.tree.command(name="setup_github_storage", description="設定 GitHub 作為資料儲存庫（首次設定）")
async def setup_github_storage(
    interaction: discord.Interaction,
    token: str,
    create_new_repo: Optional[bool] = None,
    ...
    if status:
        ...
        await interaction.followup.send(
            "\\n".join([
                "📊 GitHub 儲存狀態",
                f"✅ 啟用：{storage_cfg.get('enabled', False)}",
                f"📦 Repo：{storage_cfg.get('repo', '未設定')}",
                f"🌿 Branch：{storage_cfg.get('branch', 'main')}",
                f"🔄 自動同步：{storage_cfg.get('auto_sync', True)}",
                f"⏱️ 同步間隔：{storage_cfg.get('sync_interval', 300)} 秒",
                f"🕒 最後同步：{sync_state.get('last_sync', '尚未同步')}",
            ]),
            ephemeral=True
        )
        return
    ...
    if not token:
        await interaction.followup.send(
            "⚠️ 請提供 GitHub Personal Access Token。\n"
    ...
    if create_new_repo:
        if not repo_name:
            repo_name = f"discord-bot-storage-{int(time.time())}"
        
        await interaction.followup.send(f"🔄 正在建立新的 GitHub Repository：`{repo_name}`...", ephemeral=True)
        
        ok, result = await github_create_repo(token, repo_name, private=True)
        if ok:
            repo_full = result
            await interaction.edit_original_response(content=f"✅ 已建立 Repository：`{repo_full}`")
        else:
            await interaction.edit_original_response(content=f"❌ 建立 Repository 失敗：{result}")
            return
    elif existing_repo:
        repo_full = existing_repo.strip()
        if "/" not in repo_full:
            await interaction.followup.send("⚠️ existing_repo 格式應為 `owner/repo`", ephemeral=True)
            return
    else:
        await interaction.followup.send(
            "⚠️ 請選擇 `create_new_repo=true` 或提供 `existing_repo`（格式：owner/repo）",
            ephemeral=True
        )
        return

    config["github_storage"] = {
        "enabled": True,
        "token": token.strip(),
        "repo": repo_full,
        "branch": "main",
        "auto_sync": auto_sync if auto_sync is not None else True,
        "sync_interval": sync_interval if sync_interval is not None else 300,
    }
    save_config(config)
    ...
    await interaction.followup.send(
        "\\n".join([
            ...
            f"🔄 自動同步：{'開啟' if config['github_storage']['auto_sync'] else '關閉'}",
            f"⏱️ 同步間隔：{config['github_storage']['sync_interval']} 秒",
            "",
            "ℹ️ 系統將在背景自動同步以下檔案：",
            "- config.json（設定檔）",
            "- memory.json（對話記憶）",
            "- voice_profiles.json（語音設定）",
            "",
            "💡 使用 `/github_sync_now` 可立即手動同步",
        ]),
        ephemeral=True
    )
    ...
    ok, msg = await sync_to_github()
    if ok:
        print(f"[github_storage] 初始同步成功：{msg}")


@client.tree.command(name="github_sync_now", description="立即同步資料到 GitHub")
async def github_sync_now(interaction: discord.Interaction):
    if not is_owner(interaction):
        return
    ...
    if not storage_cfg["enabled"]:
        await interaction.followup.send("⚠️ GitHub 儲存尚未啟用，請先使用 `/setup_github_storage` 設定。", ephemeral=True)
        return
    ...
    if ok:
        ...
        await interaction.followup.send(f"❌ 同步失敗：{msg}", ephemeral=True)


@client.tree.command(name="github_load", description="從 GitHub 載入資料")
async def github_load(interaction: discord.Interaction):
    if not is_owner(interaction):
        return
    ...
    if ok:
        ...
        config = load_config()
        voice_profiles = load_voice_profiles()
        await interaction.followup.send(f"✅ {msg}\n已重新載入設定。", ephemeral=True)
    else:
        await interaction.followup.send(f"❌ 載入失敗：{msg}", ephemeral=True)


@client.tree.command(name="set_github_storage", description="修改 GitHub 儲存設定")
async def set_github_storage(
    interaction: discord.Interaction,
    enabled: Optional[bool] = None,
    auto_sync: Optional[bool] = None,
    sync_interval: Optional[int] = None,
    ...
    if clear:
        ...
        save_config(config)
        await interaction.followup.send("✅ 已清除 GitHub 儲存設定。", ephemeral=True)
        return

    storage_cfg = config.get("github_storage", {}) or {}

    if enabled is not None:
        storage_cfg["enabled"] = enabled
    if auto_sync is not None:
        storage_cfg["auto_sync"] = auto_sync
    if sync_interval is not None:
        storage_cfg["sync_interval"] = max(60, sync_interval)

    config["github_storage"] = storage_cfg
    save_config(config)
...
@client.tree.command(name="disconnect_voice", description="手動斷開語音連接（不會自動重連）")
async def disconnect_voice(interaction: discord.Interaction):
    ...
    if client.voice_client:
        try:
            ...
            pass
        client.voice_client = None
        await interaction.followup.send("✅ 已斷開語音連接。（不會自動重連，直到再次啟用語音功能）", ephemeral=True)
    else:
        await interaction.followup.send("ℹ️ 目前沒有連接任何語音頻道。", ephemeral=True)
