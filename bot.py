    ...
    if os.path.exists(CONFIG_FILE):
        try:
            with open(CONFIG_FILE, "r", encoding="utf-8") as f:
                ...
                data.setdefault("auto_chime_in", False)
...
async def request_tts_from_tonyassi(text: str, profile: dict):
    ...
    if not audio_url:
        return {"error": "tonyassi 回傳未包含音訊 URL"}
...
async def send_voice_response(channel: discord.abc.Messageable, text: str, profile: dict, target_lang: Optional[str] = None):
    ...
    if isinstance(audio_bytes, (bytes, bytearray)):
        ...
        for idx, chunk in enumerate(chunks, start=1):
            filename = f"voice_{idx}.mp3" if len(chunks) > 1 else "voice.mp3"
...
async def set_voice_listen(
    ...
    if action.value == "join":
        ...
        if parsed is None:
            await interaction.response.send_message("⚠️ 請輸入有效語音頻道 ID（數字或 #頻道）。", ephemeral=True)
            return
        ...
        if channel is None or not isinstance(channel, (discord.VoiceChannel, discord.StageChannel)):
            await interaction.response.send_message("⚠️ 找不到語音頻道或頻道類型不支援。", ephemeral=True)
            return
        ...
        try:
            ...
            await interaction.response.send_message(f"❌ 進入語音頻道失敗：{e}", ephemeral=True)
            return
        config.setdefault("voice_listen", {})["voice_channel_id"] = int(parsed)
        save_config(config)
        await interaction.response.send_message(f"✅ 已進入語音頻道：`{parsed}`", ephemeral=True)
        return
