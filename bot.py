        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(endpoint, headers=headers, json=payload, timeout=120) as resp:
                ...
                if resp.status != 200:
                    ...
                    return {"error": f"HF 錯誤 {resp.status}: {text[:200]}"}
                if "audio/mpeg" in content_type:
                    data = await resp.read()
                    return {"audio": data, "content_type": "audio/mpeg"}
...
@client.tree.command(name="voice", description="語音設定與手動觸發")
@app_commands.choices(
    action=[
